...
struct PassVaryingContext {
    ...
    CoreType GetType(u32 slot) const {
        if (slot < count) return varyings[slot].type;
...
struct IRLowering {
    ...
    // Loop nesting depth - used to ensure selection merges don't coincide with continue targets
    u32 loopDepth = 0;
    ...
    u32 breakTargetStack[MAX_LOOP_NESTING];    // Stack of break target slots (to be patched)
    u32 continueTargetStack[MAX_LOOP_NESTING]; // Stack of continue target instruction indices
    ...
    void Initialize(IRMemoryPool* memPool, const SymbolTableData* symTable, AST* astData, const char* srcBase = nullptr) {
        pool = memPool;
        symbols = symTable;
        ast = astData;
        sourceBase = srcBase;
        ...
        // Initialize unused operand slots to 0x3FFF (max register index without constant flags)
        // This value will be > registerCount so SSA will skip it, and SPIR-V backend ignores it
        ...
        program.structureInfo = (u32*)pool->Allocate(initialSize * sizeof(u32), 64);
        program.continueInfo = (u32*)pool->Allocate(initialSize * sizeof(u32), 64);
        memset(program.structureInfo, 0, initialSize * sizeof(u32));
        memset(program.continueInfo, 0xFF, initialSize * sizeof(u32));  // 0xFFFFFFFF = no continue target
        ...
        program.switchInstructionIndices = (u32*)pool->Allocate(switchCapacity * sizeof(u32), 64);
        program.switchCaseOffsets = (u32*)pool->Allocate((switchCapacity + 1) * sizeof(u32), 64);
        program.switchCaseValues = (s32*)pool->Allocate(caseCapacity * sizeof(s32), 64);
        program.switchCaseTargets = (u32*)pool->Allocate(caseCapacity * sizeof(u32), 64);
        program.switchDefaultTargets = (u32*)pool->Allocate(switchCapacity * sizeof(u32), 64);
        ...
        program.boolConstants = (u8*)pool->Allocate(8 * sizeof(u8), 64);  // Only need 2 (true/false)
        ...
        // Register info
        program.registerTypes = (u16*)pool->Allocate(MAX_REGISTERS * sizeof(u16), 64);
        ...
        // Undef register tracking (populated by SSA for variables without entry block defs)
        u32 undefCapacity = 64;  // Initial capacity
        program.undefRegs = (u16*)pool->Allocate(undefCapacity * sizeof(u16), 64);
        program.undefRegTypes = (u16*)pool->Allocate(undefCapacity * sizeof(u16), 64);
    ...
    void LowerSkip() {
        // Emit jump to continue target (will be patched when continue target is known)
        if (loopStackDepth == 0) {
    ...
    void LowerDiscard() {
        ...
        // This is a terminator instruction (like return) that ends the current block
    ...
    void LowerForCStyle(NodeRef ref) {
        ...
        if (hasBranch) {
            ...
            program.structureInfo[branchIdx] = IRProgram::PackStructure(
                IRProgram::STRUCT_LOOP_HEADER, loopEnd);
    ...
    void LowerLoop(NodeRef ref) {
        ...
        if (!loop.count.IsNull()) {
            ...
            if (!loop.untilCondition.IsNull()) {
                program.metadata[untilBranchIdx] = (continueTarget << 16) | (loopEnd & 0xFFFF);
            }
    ...
    void LowerSwitch(NodeRef ref) {
        ...
        program.structureInfo[switchIdx] = IRProgram::PackStructure(
            IRProgram::STRUCT_SWITCH_HEADER, mergePoint);
    }
    ...
    u16 LowerExpression(NodeRef ref) {
        ...
        switch (ref.Type()) {
            ...
            case ASTNodeType::ARRAY_ACCESS:
                ...
                break;
            case ASTNodeType::BLOCK: {
                // Block as expression - lower statements and capture return value as result
                const BlockData& block = ast->GetBlock(ref);
                result = 0;
                for (u32 i = 0; i < block.statements.count; i++) {
                    NodeRef stmt = block.statements[i];
                    // Check for RETURN statement - capture its value instead of emitting return
    ...
    u16 LowerUnaryOp(NodeRef ref) {
        ...
        switch (unop.op) {
            ...
            case UnaryOpType::POST_DECREMENT: {
                // x--: Save old value, subtract 1 from operand, store back, return old value
                CoreType type = GetRegisterType(operand);
                builder.EmitInstruction(OP_STORE_REG, dest, operand); // dest = old value
                SetRegisterType(dest, type);
                u16 one = EmitConstantInt(1);
    ...
    u16 LowerIdentifier(NodeRef ref) {
        ...
        // This handles undefined variables (likely a semantic error, but we need valid IR)
        u16 reg = AllocateRegister();
        variableRegisters[ident.name.nameHash] = reg;
    ...
    u32 LookupOrRegisterStructType(u32 typeNameHash) {
        ...
        // Look up in symbol table
        Symbol* typeSym = SymbolTable::LookupByHash(const_cast<SymbolTableData*>(symbols), typeNameHash);
    ...
    u32 RegisterStructTypeFromSymbol(u32 typeNameHash, const StructData& structData) {
        ...
        for (u32 i = 0; i < structData.fields.count; i++) {
            if (fieldOffset + i >= program.structFieldCapacity) break;
    ...
    CoreType InferResourceType(u32 nameHash) {
        ...
        static const u32 HASH_viewProjectionMatrix = Utils::HashStr("viewProjectionMatrix");
        ...
        static const u32 HASH_modelViewProjectionMatrix = Utils::HashStr("modelViewProjectionMatrix");
        static const u32 HASH_mvpMatrix = Utils::HashStr("mvpMatrix");
        static const u32 HASH_normalMatrix = Utils::HashStr("normalMatrix");
        static const u32 HASH_inverseViewMatrix = Utils::HashStr("inverseViewMatrix");
        static const u32 HASH_inverseProjMatrix = Utils::HashStr("inverseProjMatrix");
        static const u32 HASH_lightViewProjMatrix = Utils::HashStr("lightViewProjMatrix");
        static const u32 HASH_previousModelMatrix = Utils::HashStr("previousModelMatrix");
        static const u32 HASH_previousViewProjMatrix = Utils::HashStr("previousViewProjMatrix");
        ...
        if (nameHash == HASH_modelMatrix || nameHash == HASH_viewMatrix ||
            ...
            nameHash == HASH_viewProjectionMatrix || nameHash == HASH_viewProjMatrix ||
            nameHash == HASH_modelViewMatrix || nameHash == HASH_modelViewProjectionMatrix ||
            nameHash == HASH_mvpMatrix || nameHash == HASH_normalMatrix ||
            nameHash == HASH_inverseViewMatrix || nameHash == HASH_inverseProjMatrix ||
            nameHash == HASH_lightViewProjMatrix || nameHash == HASH_previousModelMatrix ||
            nameHash == HASH_previousViewProjMatrix || nameHash == HASH_boneMatrices) {
    ...
    u32 GetTypeSize(CoreType type) {
        switch (type) {
            ...
            case CoreType::FLOAT3: return 12;
    ...
    u32 GetTypeAlignment(CoreType type) {
        switch (type) {
            ...
            case CoreType::BOOL:   return 4;
            case CoreType::FLOAT2: return 8;
            case CoreType::FLOAT3: return 16;  // vec3 aligned to vec4
            case CoreType::FLOAT4: return 16;
            case CoreType::INT2:   return 8;
        ...
        if (target.Type() == ASTNodeType::IDENTIFIER) {
            ...
            if (access.object.Type() == ASTNodeType::IDENTIFIER) {
                ...
                if (obj.identifierKind == SpecialIdentifier::OUTPUT) {
                    ...
                    program.metadata[builder.currentInstruction - 1] = nameHash;  // Keep name for debugging
                    ...
                    if (structTypeHash != 0) {
                        ...
                        if (fieldIndex != 0xFFFFFFFF) {
                            ...
                            builder.EmitInstruction(OP_STRUCT_INSERT, newStructReg, objReg, fieldIndex, valueReg);
                    ...
                    if (componentIndex != 0xFFFFFFFF) {
                        ...
                        builder.EmitInstruction(OP_VEC_INSERT, newVecReg, objReg, componentIndex, valueReg);
                    ...
                    // Use a single shuffle operation to avoid SSA tracking issues with intermediates
                    ...
                    // 255 means "keep from original", 0-3 means "take component N from value"
                    // E.g., for "xy" assignment: positions 0,1 get value[0],value[1]; positions 2,3 stay from original
                    ...
                    for (u32 i = 0; i < sizeof(swizzlePatterns)/sizeof(swizzlePatterns[0]); i++) {
                        if (memberHash == Utils::HashStr(swizzlePatterns[i])) {
                            ...
                            for (u32 j = 0; j < numComponents; j++) {
                                ...
                                if (srcIdx != 255) {
                                    // Take from value (second vector) - index is srcIdx + numComponents
                            ...
                            builder.EmitInstruction(OP_VEC_SHUFFLE, newVecReg, objReg, valueReg);
...
void LowerIfStatement(NodeRef ref) {
    ...
    u16 LowerBinaryOp(NodeRef ref) {
        ...
        // Handle matrix operations first (before float types, since mat4 isn't in FLOAT_TYPES)
        if (leftMask & TypeMasks::MATRIX_TYPES) {
            if (binop.op == BinaryOpType::MULTIPLY) {
                if (rightMask & TypeMasks::MATRIX_TYPES) {
                    ...
                    // (use upper-left 3x3 semantically, extend with w=0, multiply, truncate)
                    op = OP_MAT_VEC_MUL;

                    u32 matDim = (leftType == CoreType::MAT4) ? 4 : (leftType == CoreType::MAT3) ? 3 : 2;
                    u32 vecDim = GetVectorDimension(rightType);

                    if (vecDim < matDim) {
                        // Vector is smaller than matrix - extend vector, multiply, truncate
                        // This handles mat4 * float3 (normal transform) and mat4 * float2, etc.
                        ...
                        CoreType extendedType = (matDim == 4) ? CoreType::FLOAT4 :
                                               (matDim == 3) ? CoreType::FLOAT3 : CoreType::FLOAT2;
                        u16 extendedVec = AllocateRegister();
                        u16 zero = builder.EmitConstant(0.0f);

                        // Construct extended vector: (vec.xyz, 0) for mat4*vec3, etc.
                        if (vecDim == 3 && matDim == 4) {
                            builder.EmitInstruction(OP_VEC_CONSTRUCT, extendedVec, right, zero);
                            program.metadata[builder.currentInstruction - 1] = 4; // 4 components
                        } else if (vecDim == 2 && matDim == 4) {
                            builder.EmitInstruction(OP_VEC_CONSTRUCT, extendedVec, right, zero, zero);
                        ...
                        builder.EmitInstruction(OP_MAT_VEC_MUL, fullResult, left, extendedVec);
                        ...
                        program.metadata[builder.currentInstruction - 1] = shuffleMask | (vecDim << 24);
                        SetRegisterType(dest, rightType);  // Result has same type as input vector
                        return dest;
                    } else {
                        // Dimensions match or vector is larger (unusual but handle gracefully)
                        // mat4 * vec4 = vec4, mat3 * vec3 = vec3, mat2 * vec2 = vec2
                        if (leftType == CoreType::MAT4) resultType = CoreType::FLOAT4;
                        else if (leftType == CoreType::MAT3) resultType = CoreType::FLOAT3;
                        else if (leftType == CoreType::MAT2) resultType = CoreType::FLOAT2;
                    }
        ...
        } else if ((leftMask & TypeMasks::FLOAT_VECTORS) && (rightMask & TypeMasks::MATRIX_TYPES)) {
            // Vector * Matrix with dimension handling
            if (binop.op == BinaryOpType::MULTIPLY) {
                op = OP_VEC_MAT_MUL;

                u32 vecDim = GetVectorDimension(leftType);
                u32 matDim = (rightType == CoreType::MAT4) ? 4 : (rightType == CoreType::MAT3) ? 3 : 2;
                ...
                if (vecDim < matDim) {
                    ...
                    if (vecDim == 3 && matDim == 4) {
                        ...
                        builder.EmitInstruction(OP_VEC_CONSTRUCT, extendedVec, left, zero, zero);
                        program.metadata[builder.currentInstruction - 1] = 4;
                    } else if (vecDim == 2 && matDim == 3) {
                        builder.EmitInstruction(OP_VEC_CONSTRUCT, extendedVec, left, zero);
                        program.metadata[builder.currentInstruction - 1] = 3;
                    ...
                    // Vector * Matrix
                    u16 fullResult = AllocateRegister();
                    builder.EmitInstruction(OP_VEC_MAT_MUL, fullResult, extendedVec, right);
                    SetRegisterType(fullResult, extendedType);
        ...
        // Handle vector dimension mismatch: if both operands are vectors but with different
        // dimensions, truncate the larger vector to match the smaller one.
        ...
        if (leftDim > 1 && rightDim > 1 && leftDim != rightDim) {
            ...
            if (leftDim > targetDim) {
                ...
                // The metadata encodes the shuffle mask: component indices 0, 1, 2 for vec3
                u32 shuffleMask = 0;
                ...
                program.metadata[builder.currentInstruction - 1] = shuffleMask | (targetDim << 24);
        ...
        if (typeMask & TypeMasks::FLOAT_TYPES) {
            switch (binop.op) {
                case BinaryOpType::ADD:        op = OP_FADD; break;
                case BinaryOpType::SUBTRACT:   op = OP_FSUB; break;
                case BinaryOpType::MULTIPLY:   op = OP_FMUL; break;
                case BinaryOpType::DIVIDE:     op = OP_FDIV; break;
                case BinaryOpType::MODULO:     op = OP_FMOD; break;
                case BinaryOpType::EQUALS:     op = OP_FEQ;  break;
                case BinaryOpType::NOT_EQUALS: op = OP_FNE;  break;
                case BinaryOpType::LESS:       op = OP_FLT;  break;
                case BinaryOpType::LESS_EQUAL: op = OP_FLE;  break;
                case BinaryOpType::GREATER:    op = OP_FGT;  break;
                case BinaryOpType::GREATER_EQUAL: op = OP_FGE; break;
                case BinaryOpType::AND:        op = OP_AND;  break;
                case BinaryOpType::OR:         op = OP_OR;   break;
                default:                       op = OP_NOP;  break;
            }
        } else if (typeMask & TypeMasks::UINT_TYPES) {
            switch (binop.op) {
                case BinaryOpType::ADD:        op = OP_IADD; break;
                case BinaryOpType::SUBTRACT:   op = OP_ISUB; break;
                case BinaryOpType::MULTIPLY:   op = OP_IMUL; break;
                case BinaryOpType::DIVIDE:     op = OP_IDIV; break;  // Note: should use UDIV
                case BinaryOpType::MODULO:     op = OP_IMOD; break;
                case BinaryOpType::EQUALS:     op = OP_IEQ;  break;
                case BinaryOpType::NOT_EQUALS: op = OP_INE;  break;
                case BinaryOpType::LESS:       op = OP_ULT;  break;
                case BinaryOpType::LESS_EQUAL: op = OP_ULE;  break;
                case BinaryOpType::GREATER:    op = OP_UGT;  break;
                case BinaryOpType::GREATER_EQUAL: op = OP_UGE; break;
                case BinaryOpType::AND:        op = OP_AND;  break;
                case BinaryOpType::OR:         op = OP_OR;   break;
                case BinaryOpType::BITWISE_AND: op = OP_AND; break;
                case BinaryOpType::BITWISE_OR:  op = OP_OR;  break;
                case BinaryOpType::BITWISE_XOR: op = OP_XOR; break;
                case BinaryOpType::LEFT_SHIFT:  op = OP_SHL; break;
                case BinaryOpType::RIGHT_SHIFT: op = OP_SHR; break;
                default:                       op = OP_NOP;  break;
            }
            ...
            switch (binop.op) {
                ...
                case BinaryOpType::SUBTRACT:   op = OP_ISUB; break;
                case BinaryOpType::MULTIPLY:   op = OP_IMUL; break;
                case BinaryOpType::DIVIDE:     op = OP_IDIV; break;
                case BinaryOpType::MODULO:     op = OP_IMOD; break;
                case BinaryOpType::EQUALS:     op = OP_IEQ;  break;
                case BinaryOpType::NOT_EQUALS: op = OP_INE;  break;
                case BinaryOpType::LESS:       op = OP_ILT;  break;
                case BinaryOpType::LESS_EQUAL: op = OP_ILE;  break;
                case BinaryOpType::GREATER:    op = OP_IGT;  break;
                case BinaryOpType::GREATER_EQUAL: op = OP_IGE; break;
                case BinaryOpType::AND:        op = OP_AND;  break;
                case BinaryOpType::OR:         op = OP_OR;   break;
                case BinaryOpType::BITWISE_AND: op = OP_AND; break;
                case BinaryOpType::BITWISE_OR:  op = OP_OR;  break;
                case BinaryOpType::BITWISE_XOR: op = OP_XOR; break;
                case BinaryOpType::LEFT_SHIFT:  op = OP_SHL; break;
                case BinaryOpType::RIGHT_SHIFT: op = OP_ASR; break; // Arithmetic shift for signed
                default:                       op = OP_NOP;  break;
            }
        } else {
            // Bool or other types
        ...
        // Comparison operations produce BOOL, other operations produce the operand type
        ...
        if (!isComparison) {
            ...
            bool rightIsVector = (rightMask & (TypeMasks::FLOAT_VECTORS | TypeMasks::INT_VECTORS | TypeMasks::UINT_VECTORS)) != 0;
    ...
    u16 LowerMemberAccess(NodeRef ref) {
        ...
        if (access.object.Type() == ASTNodeType::MEMBER_ACCESS) {
            ...
            // Check if the object register has a struct type (e.g., resources.lights -> LightSourcesSoA)
            u32 structTypeHash = 0;
            ...
            if (structTypeHash != 0) {
                ...
                if (structIt != structTypeMap.end()) {
                    ...
                    for (u32 i = 0; i < info.fieldCount; i++) {
                        if (program.structFieldNameHashes[info.fieldOffset + i] == access.member.nameHash) {
                            fieldIndex = i;
                            fieldType = static_cast<CoreType>(program.structFieldTypes[info.fieldOffset + i]);
                            break;
                        }
                    }
                }
                ...
                if (fieldIndex != 0xFFFFFFFF) {
                    ...
                    if (isStoragePtr) {
                        // Storage buffer field access - emit OP_STORAGE_FIELD to maintain pointer semantics
                        builder.EmitInstruction(OP_STORAGE_FIELD, dest, objectReg, fieldIndex);
                        program.metadata[builder.currentInstruction - 1] = structTypeHash;
            ...
            // Then extract the component based on the member name (x/y/z/w or r/g/b/a)
            u32 memberHash = access.member.nameHash;
            u32 componentIndex = 0xFFFFFFFF;
        ...
        if (access.object.Type() == ASTNodeType::FUNCTION_CALL) {
            ...
            if (memberHash == HASH_RGB || memberHash == HASH_XYZ) {
                ...
                u16 dest = AllocateRegister();
                // For VEC_SHUFFLE: operand0 = src0, operand1 = src1 (same for single-source swizzle)
                // Metadata = shuffle mask (4 bits per component)
        ...
        switch (obj.identifierKind) {
            ...
            case SpecialIdentifier::RESOURCES: {
                ...
                if (!sym || sym->kind != SymbolKind::RESOURCE) return 0;
                ...
                switch (memberHash) {
                    case BuiltinHash::VERTEX_ID:
                        if (currentStage != ShaderStage::Vertex) {
                            fprintf(stderr, "Error: input.vertex_id is only available in vertex shaders\n");
                            return 0;
                        }
                        builder.EmitInstruction(OP_LOAD_INPUT, dest, BuiltinInputSlot::VERTEX_ID);
                        SetRegisterType(dest, CoreType::UINT);
                        return dest;

                    case BuiltinHash::INSTANCE_ID:
                        if (currentStage != ShaderStage::Vertex) {
                            fprintf(stderr, "Error: input.instance_id is only available in vertex shaders\n");
                            return 0;
                        }
                        builder.EmitInstruction(OP_LOAD_INPUT, dest, BuiltinInputSlot::INSTANCE_ID);
                        SetRegisterType(dest, CoreType::UINT);
                        return dest;

                    case BuiltinHash::GLOBAL_ID:
                        if (currentStage != ShaderStage::Compute) {
                            fprintf(stderr, "Error: input.global_id is only available in compute shaders\n");
                            return 0;
                        }
                    ...
                    case BuiltinHash::LOCAL_ID:
                        if (currentStage != ShaderStage::Compute) {
                            fprintf(stderr, "Error: input.local_id is only available in compute shaders\n");
                            return 0;
                        }
                    ...
                    case BuiltinHash::WORKGROUP_ID:
                        if (currentStage != ShaderStage::Compute) {
                            fprintf(stderr, "Error: input.workgroup_id is only available in compute shaders\n");
                            return 0;
                        }
                    ...
                    case BuiltinHash::NUM_WORKGROUPS:
                        if (currentStage != ShaderStage::Compute) {
                            fprintf(stderr, "Error: input.num_workgroups is only available in compute shaders\n");
                            return 0;
                        }
                    ...
                    case BuiltinHash::LOCAL_INDEX:
                        if (currentStage != ShaderStage::Compute) {
                            fprintf(stderr, "Error: input.local_index is only available in compute shaders\n");
                            return 0;
                        }
            ...
            case SpecialIdentifier::NONE: {
                ...
                u32 componentIndex = 0xFFFFFFFF;
                if (access.member.nameHash == Utils::HashStr("x") || access.member.nameHash == Utils::HashStr("r")) componentIndex = 0;
                else if (access.member.nameHash == Utils::HashStr("y") || access.member.nameHash == Utils::HashStr("g")) componentIndex = 1;
                else if (access.member.nameHash == Utils::HashStr("z") || access.member.nameHash == Utils::HashStr("b")) componentIndex = 2;
                else if (access.member.nameHash == Utils::HashStr("w") || access.member.nameHash == Utils::HashStr("a")) componentIndex = 3;
    ...
    u16 LowerFunctionCall(NodeRef ref) {
        ...
        if (!call.moduleObject.IsNull() && call.moduleObject.Type() == ASTNodeType::IDENTIFIER) {
            ...
            Symbol* enumSym = SymbolTable::LookupByHash(const_cast<SymbolTableData*>(symbols), enumHash);
            if (enumSym && (enumSym->kind == SymbolKind::ENUM || enumSym->kind == SymbolKind::ENUM_SYMBOL)) {
                ...
                if (variantIndex != 0xFFFFFFFF) {
                    ...
                    builder.EmitInstruction(OP_ENUM_CONSTRUCT, dest, args[0], args[1], args[2], args[3]);
                    // metadata = (variantIndex << 16) | (argCount << 8) | enumStructHash lower bits
        ...
        if (call.flags & FunctionCallFlags::IS_METHOD_CALL) {
            ...
            if (!call.moduleObject.IsNull()) {
                ...
                if (receiverStructHash != 0) {
                    ...
                    // ENUM for module enums, ENUM_SYMBOL for pipeline-local enums
                    Symbol* enumSym = SymbolTable::LookupByHash(const_cast<SymbolTableData*>(symbols), receiverStructHash);
                    if (enumSym && (enumSym->kind == SymbolKind::ENUM || enumSym->kind == SymbolKind::ENUM_SYMBOL)) {
                        const EnumData& enumData = symbols->enums[enumSym->index];
                        ...
                        if (!methodRef.IsNull()) {
                            ...
                            if (method.body.Type() == ASTNodeType::PATTERN_MATCH) {
                                ...
                                // Non-pattern-match method body (e.g., normal method that calls self.distance)
                                // Inline the method body similar to regular function inlining
        ...
        if (call.flags & FunctionCallFlags::IS_INTRINSIC) {
            ...
            switch (intrinsic) {
                ...
                case Intrinsic::MIN: {
                    CoreType argType = GetRegisterType(args[0]);
                    if (mask(argType) & TypeMasks::FLOAT_TYPES) op = OP_FMIN;
                    else if (mask(argType) & TypeMasks::UINT_TYPES) op = OP_UMIN;
                    else op = OP_IMIN;
                    break;
                }
                case Intrinsic::MAX: {
                    CoreType argType = GetRegisterType(args[0]);
                    if (mask(argType) & TypeMasks::FLOAT_TYPES) op = OP_FMAX;
                    else if (mask(argType) & TypeMasks::UINT_TYPES) op = OP_UMAX;
                    else op = OP_IMAX;
                    break;
                }
                case Intrinsic::CLAMP: {
                    CoreType argType = GetRegisterType(args[0]);
                    if (mask(argType) & TypeMasks::FLOAT_TYPES) op = OP_FCLAMP;
                    else if (mask(argType) & TypeMasks::UINT_TYPES) op = OP_UCLAMP;
                    else op = OP_ICLAMP;
                    break;
                }
                case Intrinsic::MOD: {
            ...
            if (constructedType != CoreType::INVALID && constructedType != CoreType::VOID) {
                ...
                bool isScalarConversion = (constructedType == CoreType::FLOAT ||
                                           ...
                                           constructedType == CoreType::UINT) && argCount == 1;
                ...
                if (isScalarConversion) {
                    ...
                    if (constructedType == CoreType::FLOAT) {
                        if (srcType == CoreType::INT) convOp = OP_I2F;
                        else if (srcType == CoreType::UINT) convOp = OP_U2F;
    ...
    // Try to inline a function call, returns the result register or 0xFFFF if inlining failed
    u16 TryInlineFunction(const FunctionCallData& call, u16* args, u32 argCount) {
        ...
        if (funcRef.IsNull() && call.moduleQualifiedHash != 0) {
            for (u32 m = 0; m < ast->modules.count; m++) {
                ...
                if (!funcRef.IsNull()) break;
            }
        }

        // For unqualified calls during inlining, search in the current inline module
        if (funcRef.IsNull() && inlineModuleIndex != 0xFFFFFFFF && inlineModuleIndex < ast->modules.count) {
        ...
        // Try looking up in the current pass's function list (pass-scoped functions)
        ...
        // Save current variable register mappings and node register cache
        // Node register cache must be saved because we're re-lowering the same AST nodes
        // and each inlining needs fresh computations (not cached results from previous inlining)
        auto savedVariableRegisters = variableRegisters;
        auto savedNodeRegisters = nodeRegisters;

        // Bind parameters to argument registers
        u32 paramCount = func.parameters.count < argCount ? func.parameters.count : argCount;
        ...
        // Set return type
        if (func.returnType != CoreType::INVALID && func.returnType != CoreType::VOID) {
    ...
    u32 GetInputSlotIndex(u32 nameHash) {
        ...
        //
        // IMPORTANT: Slots must match vertex output slots which use VARYING0 + index
        // so fragment inputs must also use VARYING0 + index for locations to match
        ...
        if (nameHash == NORMAL_HASH) return OutputSlot::VARYING0 + 0;
        if (nameHash == TEXCOORD_HASH) return OutputSlot::VARYING0 + 1;
        if (nameHash == COLOR_HASH) return OutputSlot::VARYING0 + 2;
        if (nameHash == TANGENT_HASH) return OutputSlot::VARYING0 + 3;
        if (nameHash == WORLDPOS_HASH) return OutputSlot::VARYING0 + 4;
        return OutputSlot::VARYING0;
    }

    // Get the type of a fragment shader input (varying) from the pipeline attributes
    CoreType GetInputTypeFromAttribute(u32 nameHash) {
        if (currentPipeline.IsNull()) return CoreType::FLOAT3;
        ...
        for (u32 i = 0; i < pipeline.attributes.count; i++) {
            ...
            if (attrRef.Type() == ASTNodeType::ATTRIBUTE_DECL) {
                ...
                if (attrDecl.name.nameHash == nameHash) {
                    ...
                    if (typeHash == FLOAT_HASH) return CoreType::FLOAT;
                    if (typeHash == FLOAT2_HASH) return CoreType::FLOAT2;
                    if (typeHash == FLOAT3_HASH) return CoreType::FLOAT3;
                    if (typeHash == FLOAT4_HASH) return CoreType::FLOAT4;
                    if (typeHash == INT_HASH) return CoreType::INT;
                    if (typeHash == INT2_HASH) return CoreType::INT2;
                    if (typeHash == INT3_HASH) return CoreType::INT3;
                    if (typeHash == INT4_HASH) return CoreType::INT4;
                    if (typeHash == UINT_HASH) return CoreType::UINT;
                    if (typeHash == UINT2_HASH) return CoreType::UINT2;
                    if (typeHash == UINT3_HASH) return CoreType::UINT3;
                    if (typeHash == UINT4_HASH) return CoreType::UINT4;
                }
            }
        }
    ...
    CompressionFormat GetAttributeCompression(u32 attrIndex) {
        if (currentPipeline.IsNull()) return CompressionFormat::NONE;
    ...
    CoreType GetRegisterType(u16 reg) {
        ...
        if ((reg & 0xC000) == 0xC000) return CoreType::BOOL;  // Bool constant (0xC000 prefix)
        if (reg & 0x8000) return CoreType::FLOAT;   // Float constant (0x8000 prefix)
        if (reg & 0x4000) return CoreType::INT;     // Int constant (0x4000 prefix)
        if (reg & 0x2000) return CoreType::UINT;    // Uint constant (0x2000 prefix)
    ...
    CoreType GetVectorType(CoreType scalarType, int componentCount) {
        switch (scalarType) {
            case CoreType::FLOAT:
                if (componentCount == 2) return CoreType::FLOAT2;
                if (componentCount == 3) return CoreType::FLOAT3;
                if (componentCount == 4) return CoreType::FLOAT4;
                break;
            case CoreType::INT:
                if (componentCount == 2) return CoreType::INT2;
                if (componentCount == 3) return CoreType::INT3;
                if (componentCount == 4) return CoreType::INT4;
                break;
            case CoreType::UINT:
                if (componentCount == 2) return CoreType::UINT2;
                if (componentCount == 3) return CoreType::UINT3;
                if (componentCount == 4) return CoreType::UINT4;
                break;
            case CoreType::BOOL:
                if (componentCount == 2) return CoreType::BOOL2;
                if (componentCount == 3) return CoreType::BOOL3;
                if (componentCount == 4) return CoreType::BOOL4;
                break;
            default:
                break;
        }
    ...
    CoreType InferExpressionType(NodeRef ref) {
        if (ref.IsNull()) return CoreType::INT;  // Default
        ...
        switch (ref.Type()) {
            case ASTNodeType::LITERAL: {
                ...
                switch (lit.value.type) {
                    case LiteralValue::FLOAT: return CoreType::FLOAT;
                    case LiteralValue::INT:   return CoreType::INT;
                    case LiteralValue::UINT:  return CoreType::UINT;
                    case LiteralValue::BOOL:  return CoreType::BOOL;
                    default:                  return CoreType::INT;
                }
            }
            case ASTNodeType::IDENTIFIER: {
                const IdentifierData& ident = ast->GetIdentifier(ref);
    ...
    // Get the number of components for a vector type (1 for scalars, 2-4 for vectors)
    u32 GetVectorDimension(CoreType type) {
        switch (type) {
            case CoreType::FLOAT2: case CoreType::INT2: case CoreType::UINT2: case CoreType::BOOL2:
                return 2;
            case CoreType::FLOAT3: case CoreType::INT3: case CoreType::UINT3: case CoreType::BOOL3:
                return 3;
            case CoreType::FLOAT4: case CoreType::INT4: case CoreType::UINT4: case CoreType::BOOL4:
                return 4;
            default:
    ...
    CoreType GetVectorTypeWithDimension(CoreType baseType, u32 dim) {
        ...
        bool isFloat = (baseType == CoreType::FLOAT || baseType == CoreType::FLOAT2 ||
        ...
        if (isFloat) {
            switch (dim) {
                case 1: return CoreType::FLOAT;
                case 2: return CoreType::FLOAT2;
                case 3: return CoreType::FLOAT3;
                case 4: return CoreType::FLOAT4;
            }
        } else if (isInt) {
            switch (dim) {
                case 1: return CoreType::INT;
                case 2: return CoreType::INT2;
                case 3: return CoreType::INT3;
                case 4: return CoreType::INT4;
            }
        } else if (isUint) {
            switch (dim) {
                case 1: return CoreType::UINT;
                case 2: return CoreType::UINT2;
                case 3: return CoreType::UINT3;
                case 4: return CoreType::UINT4;
            }
        }
...
OpCode IntrinsicToOpcode(StdLib::Intrinsic intrinsic) {
    ...
    switch (intrinsic) {
        // Math
        case Intrinsic::LERP:         return OP_LERP;
        case Intrinsic::SMOOTHSTEP:   return OP_SMOOTHSTEP;
        case Intrinsic::SATURATE:     return OP_SATURATE;
        case Intrinsic::FRACT:        return OP_FRACT;
        case Intrinsic::STEP:         return OP_STEP;
        case Intrinsic::SIGN:         return OP_SIGN;    
        case Intrinsic::FLOOR:        return OP_FLOOR;
        case Intrinsic::CEIL:         return OP_CEIL;
        case Intrinsic::ROUND:        return OP_ROUND;
        case Intrinsic::POW:          return OP_POW;
        case Intrinsic::SQRT:         return OP_SQRT;
        case Intrinsic::RSQRT:        return OP_RSQRT;
        case Intrinsic::EXP:          return OP_EXP;
        case Intrinsic::EXP2:         return OP_EXP2;
        case Intrinsic::LOG:          return OP_LOG;
        case Intrinsic::LOG2:         return OP_LOG2;
        
        // Trigonometry
        case Intrinsic::SIN:          return OP_SIN;
        case Intrinsic::COS:          return OP_COS;
        case Intrinsic::TAN:          return OP_TAN;
        case Intrinsic::ASIN:         return OP_ASIN;
        case Intrinsic::ACOS:         return OP_ACOS;
        case Intrinsic::ATAN:         return OP_ATAN;
        case Intrinsic::ATAN2:        return OP_ATAN2;
        case Intrinsic::DEGREES:      return OP_DEGREES;    
        case Intrinsic::RADIANS:      return OP_RADIANS;     
        
        // Vector
        case Intrinsic::DOT:          return OP_DOT;
        case Intrinsic::CROSS:        return OP_CROSS;
        case Intrinsic::NORMALIZE:    return OP_NORMALIZE;
        case Intrinsic::LENGTH:       return OP_LENGTH;
        case Intrinsic::DISTANCE:     return OP_DISTANCE;
        case Intrinsic::REFLECT:      return OP_REFLECT;
        case Intrinsic::REFRACT:      return OP_REFRACT;
        case Intrinsic::FACEFORWARD:  return OP_FACEFORWARD;
        
        // Matrix
        case Intrinsic::TRANSPOSE:    return OP_MAT_TRANSPOSE;
        case Intrinsic::DETERMINANT:  return OP_MAT_DET;
        case Intrinsic::INVERSE:      return OP_MAT_INVERSE;
        
        // Derivatives (fragment only)
        case Intrinsic::DDX:          return OP_DDX;
        case Intrinsic::DDY:          return OP_DDY;
        case Intrinsic::DDX_FINE:     return OP_DDX_FINE;
        case Intrinsic::DDY_FINE:     return OP_DDY_FINE;
        case Intrinsic::DDX_COARSE:   return OP_DDX_COARSE;
        case Intrinsic::DDY_COARSE:   return OP_DDY_COARSE;
        case Intrinsic::FWIDTH:       return OP_FWIDTH;
        
        // Texture operations - these need special handling
        case Intrinsic::SAMPLE:       return OP_TEX_SAMPLE;
        case Intrinsic::SAMPLE_LOD:   return OP_TEX_SAMPLE_LOD;
        case Intrinsic::SAMPLE_BIAS:  return OP_TEX_SAMPLE_BIAS;
        case Intrinsic::SAMPLE_GRAD:  return OP_TEX_SAMPLE_GRAD;
        case Intrinsic::SAMPLE_CMP:   return OP_TEX_SAMPLE_CMP;
        ...
        case Intrinsic::WAVE_ACTIVE_PRODUCT:  return OP_WAVE_MUL;
        case Intrinsic::WAVE_ACTIVE_MIN:      return OP_WAVE_MIN; 
        case Intrinsic::WAVE_ACTIVE_MAX:      return OP_WAVE_MAX;  
        case Intrinsic::WAVE_ACTIVE_ALL:      return OP_WAVE_ALL;
        case Intrinsic::WAVE_ACTIVE_ANY:      return OP_WAVE_ANY;
        case Intrinsic::WAVE_BROADCAST:       return OP_WAVE_READ_LANE;
        case Intrinsic::WAVE_READ_FIRST:      return OP_WAVE_READ_FIRST;
        
        // Atomics
        case Intrinsic::ATOMIC_ADD:           return OP_ATOMIC_ADD;
        case Intrinsic::ATOMIC_MIN:           return OP_ATOMIC_MIN;
        case Intrinsic::ATOMIC_MAX:           return OP_ATOMIC_MAX;
        case Intrinsic::ATOMIC_AND:           return OP_ATOMIC_AND;
        case Intrinsic::ATOMIC_OR:            return OP_ATOMIC_OR;
        case Intrinsic::ATOMIC_XOR:           return OP_ATOMIC_XOR;
        case Intrinsic::ATOMIC_EXCHANGE:      return OP_ATOMIC_XCHG;
        case Intrinsic::ATOMIC_CMP_EXCHANGE:  return OP_ATOMIC_CMP_XCHG;
        
        // Bit operations
        case Intrinsic::COUNT_BITS:           return OP_POPCNT;
        case Intrinsic::REVERSE_BITS:         return OP_REVERSE_BITS;
        case Intrinsic::FIRST_BIT_LOW:        return OP_CTZ;
        case Intrinsic::FIRST_BIT_HIGH:       return OP_CLZ;

        // Control flow
        case Intrinsic::SELECT:               return OP_SELECT;

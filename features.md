...
This document inventories the BWSL language surface implemented in this repository, with a maturity assessment and a note on what makes each feature distinctive. It is based on `docs/spec/`, `docs/language.md`, `bwsl_token_defs.h`, `bwsl_parser_soa.cpp`, `bwsl_types.h`, `bwsl_stdlib.h`, `bwsl_ir_lowering.h`, `bwsl_compute_graph.cpp`, `docs/render-config.md`, the standard modules, and the test corpus.
...
| Legacy resource aliases | Implementation-defined | Older render-config forms such as `sampler2D`, `samplerCube`, `sampler3D`, `sampler2DArray` appear in tests and compatibility paths. | Preserves older shader style while newer code moves toward explicit texture plus sampler resources. |
...
| Control flow | Stable | Blocks, `if`/`else`, `else if`, single-statement bodies, C-style `for`, range `for`, collection `for`, `foreach`, `loop`, `switch`, `return`, `break`, `skip`, `discard`. | Includes several shader-friendly loop forms and conditional jump variants without macros. |
...
| Legacy render config | Stable but separate from source spec | `.rcfg` files define targets, uniforms, textures, samplers, buffers, images, pass types, attachments, dependencies, dispatch, and instancing. | Supports engine integration while the source language evolves toward explicit resources. |

...
#include "core/bwsl_utils.h"  // For Utils::HashStr
#include <bit>
...
namespace OutputSlot {
    constexpr u8 POSITION = 0;       // output.position -> BuiltIn Position
    constexpr u8 COLOR    = 1;       // output.color -> Location 0 (fragment)
    constexpr u8 COLOR0   = COLOR;
    constexpr u8 VARYING0 = 2;       // First varying output
    constexpr u8 VARYING1 = 3;
    constexpr u8 VARYING2 = 4;
    constexpr u8 VARYING3 = 5;
    // Fragment depth
    constexpr u8 DEPTH    = 16;      // output.depth -> BuiltIn FragDepth

    inline constexpr u8 FragmentColor(u8 location) {
        return COLOR0 + location;
    }

    inline constexpr u8 FragmentColorLocation(u8 slot) {
        return slot - COLOR0;
    }
}
